...
if(isset($_POST['js']) && $_POST['js'] === "on") {
    ...
    if ($good == 'good' || $good == 'nogood')
    {
        ...
        else
        {
            ...
            // Morgan: 좋아요 알림 (추천만, 비추천은 알림 안 함)
            if ($good === 'good' && function_exists('mg_notify') && !empty($write['mb_id']) && $write['mb_id'] !== $member['mb_id']) {
                $noti_url = get_pretty_url($bo_table, $wr_id);
                $noti_subject = get_text(stripslashes($write['wr_subject']), 0, 50);
                mg_notify(
                    $write['mb_id'],
                    'like',
                    $member['mb_nick'] . '님이 회원님의 글을 추천했습니다.',
                    $noti_subject,
                    $noti_url
                );
            }
