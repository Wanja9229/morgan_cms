...
if ($mb_nick != $mb['mb_nick']) {
    $dup = sql_fetch("SELECT mb_id FROM {$g5['member_table']} WHERE mb_nick = '{$mb_nick}' AND mb_id != '{$mb_id}'");
    if ($dup['mb_id']) { alert('이미 사용 중인 닉네임입니다.'); }
}

// 업데이트 쿼리
$sql_set = "mb_nick = '{$mb_nick}',
            mb_name = '{$mb_name}',
            mb_email = '{$mb_email}',
            mb_level = {$mb_level},
            mb_memo = '{$mb_memo}',
...
sql_query("UPDATE {$g5['member_table']} SET {$sql_set} WHERE mb_id = '{$mb_id}'");
