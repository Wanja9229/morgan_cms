...
| 마이룸 | 아이소메트릭 2D 방 꾸미기 | 상점 시스템 (가구 아이템) |
...
#### 마이룸 기본
- 아이소메트릭 2D 그리드 기반 렌더링
- 가구 배치/회전/제거
- 벽지/바닥 테마 변경
- 방 크기 확장 (포인트 또는 상점 아이템)

#### 가구 시스템
- 카테고리별 가구 (침대, 책상, 조명, 장식 등)
- 상점에서 가구 구매 → 인벤토리 → 마이룸 배치
- 가구별 아이소메트릭 이미지 (관리자 업로드)

### 4.3 Morgan CMS 연동 사항

| 연동 대상 | 연동 내용 |
|----------|----------|
| **상점 시스템** | 가구 아이템 구매 (furniture 타입 이미 존재) |
| **인벤토리** | 보유 가구 목록, 배치/회수 관리 |
| **캐릭터 시스템** | 캐릭터별 마이룸 (선택적) |
...
| room_id | PK |
| mb_id | 소유자 |
| room_size | 방 크기 (기본 6x6) |
| wallpaper | 벽지 ID |
| floor | 바닥 ID |

#### mg_furniture_own (가구 소유 + 배치)
| 필드 | 설명 |
|------|------|
| fo_id | PK |
| mb_id | 소유자 |
