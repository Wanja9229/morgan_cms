...
> 추가일: 2026-02-20
> 메인 페이지 빌더의 GridStack + CSS Grid 패턴을 인장 편집에 재활용.
> 기존 고정 레이아웃 → 2D 격자 캔버스 자유 배치로 전환.

### 기획 의도

기존 인장은 모든 유저가 동일한 레이아웃(좌:캐릭터, 중:텍스트, 우:트로피)을 사용.
그리드 빌더를 도입하면:

- **자기 표현 극대화**: 같은 요소를 가져도 배치가 다르면 완전히 다른 카드
- **상점 아이템 가치 상승**: 꾸미기 스티커를 "어디에 놓을지"까지 고민 → 구매 동기 강화
- **업적 쇼케이스 활성화**: 트로피를 원하는 위치에 강조 배치 가능
- **코드 재사용**: 메인 빌더의 GridStack 초기화, 저장/로드, CSS Grid 렌더링 로직 그대로 활용
...
| 가로 칸 수 | 10 | 고정 (커스텀 불가) |
| 세로 칸 수 | 6 | 고정 |
| 셀 비율 | 정사각형 | 컨테이너 너비 / 10 = 셀 높이 |
| 최대 카드 크기 | ~600×360px | 10×6 정사각형 기준 |

### 배치 가능 요소

| 요소 | 출처 | 기본 크기 | 비고 |
|------|------|:---:|------|
| 캐릭터 이미지 | 대표 캐릭터 (ch_main=1) | 2×3 | 썸네일 자동 |
| 닉네임 | g5_member.mb_nick | 3×1 | 텍스트, 색상 선택 가능 |
| 칭호 | mg_item_active (title) | 2×1 | 착용 중인 칭호 자동 |
| 한마디 | 직접 입력 | 4×1 | 최대 50자 |
| 자유 텍스트 | 직접 입력 | 4×2 | 최대 300자 |
| 이미지 | 업로드/URL | 3×2 | 기존 seal_image |
| 링크 버튼 | 직접 입력 | 2×1 | URL + 텍스트 |
| 트로피 | mg_user_achievement_display | 1×1 각 | 최대 5개 |
| 꾸미기 스티커 | 상점 구매 (seal_sticker) | 1×1~2×2 | **신규 아이템 타입** |
| 배경/프레임 | 상점 구매 (기존) | 전체 | 격자 위에 레이어 |

### 데이터 모델

`mg_seal` 테이블에 `seal_layout` TEXT 컬럼 추가:

```json
{
  "elements": [
    {"type": "character", "x": 0, "y": 0, "w": 2, "h": 3},
    {"type": "nickname",  "x": 2, "y": 0, "w": 3, "h": 1, "color": "#f2f3f5"},
    {"type": "title",     "x": 5, "y": 0, "w": 2, "h": 1},
    {"type": "tagline",   "x": 2, "y": 1, "w": 5, "h": 1},
    {"type": "text",      "x": 2, "y": 2, "w": 4, "h": 2},
    {"type": "trophy",    "x": 8, "y": 0, "w": 1, "h": 1, "slot": 1},
    {"type": "trophy",    "x": 9, "y": 0, "w": 1, "h": 1, "slot": 2},
    {"type": "sticker",   "x": 7, "y": 4, "w": 2, "h": 2, "item_id": 123}
  ]
}
```

- `seal_layout`이 비어있으면 기존 고정 레이아웃으로 폴백 (하위 호환)
- 기존 `seal_tagline`, `seal_content` 등 필드는 데이터 소스로 유지
- 레이아웃 JSON은 위치만 저장, 실제 콘텐츠는 기존 필드에서 읽음

### 편집기 (PC 전용)

- **GridStack.js** 사용 (메인 빌더와 동일)
- 격자: 10열 × 6행, `float: true` (자유 배치)
- **2~3배 확대** 표시 → 옆에 실제 크기 미리보기
- 요소 추가: 사이드 패널에서 드래그 또는 클릭으로 배치
- 요소 삭제: 위젯 내 삭제 버튼
- 저장: `seal_layout` JSON으로 AJAX 저장

### 모바일 대응

- **편집 불가**: 모바일 접속 시 "PC에서 편집해주세요" 안내 메시지
- **렌더링은 가능**: CSS Grid로 출력, 모바일에서도 정상 표시 (축소 비율)

### 렌더링 (mg_render_seal 수정)

**full 모드** → CSS Grid 기반:

```php
// seal_layout이 있으면 그리드 렌더링
$layout = json_decode($seal['seal_layout'], true);
if (!empty($layout['elements'])) {
    // CSS Grid: grid-template-columns: repeat(10, 1fr)
    // 각 요소를 grid-column/grid-row으로 배치
    // 인라인 JS로 정사각형 셀 높이 계산
}
```

**compact 모드** → 기존 고정 레이아웃 유지:

```
[썸네일] 닉네임 ★칭호★ "한마디..."
```

compact 모드는 그리드 레이아웃과 무관하게 항상 동일한 형태로 표시.

### 렌더링 위치 변경

| 위치 | 변경 |
|------|------|
| 게시글 view (일부 스킨) | full 모드 유지 |
| character_view.php | full 모드 유지 |
| rp_api.php | **인장 표시 제거** (seal_show_in_rp = 0 기본값) |
| mypage.php | compact 모드 유지 (그리드 미적용) |
| seal_edit.php | 편집기 미리보기 (full 모드) |

### 신규 상점 아이템

| 타입 코드 | 설명 | si_effect 예시 |
|-----------|------|---------------|
| `seal_sticker` | 인장 꾸미기 스티커 | `{"type":"seal_sticker","image":"/data/shop/sticker_001.png","default_w":1,"default_h":1}` |

- 스티커는 인벤토리에 보유 → 인장 편집기에서 배치
- 같은 스티커를 여러 개 배치 가능 (인벤토리 1개 = 무한 사용)
- 기본 크기 1×1, 유저가 리사이즈 가능 (최대 2×2)

### 성능 고려

한 페이지에 최대 1개 인장만 렌더링 (뷰페이지/캐릭터 상세).
CSS Grid 단일 요소이므로 성능 부담 없음. 캐싱/이미지 변환 불필요.

### 확장 가능성

| 확장 | 설명 |
|------|------|
| 인장 템플릿 공유 | 레이아웃 JSON을 복사/공유하여 다른 유저 레이아웃 적용 |
| 시즌 한정 레이아웃 | 이벤트 시 특수 격자 크기 (예: 12×4 와이드) 제공 |
| 인장 갤러리 | 다른 유저 인장 구경 + 좋아요 |
| 마이룸(2차-D) 연계 | 마이룸도 동일한 그리드 빌더 사용 가능 |

### 구현 순서

```
[1] DB 변경: mg_seal에 seal_layout 컬럼 추가
[2] 편집기 UI: GridStack 10×6 캔버스 + 요소 패널 + 미리보기
[3] 저장/로드: seal_layout JSON CRUD
[4] 렌더링: mg_render_seal() full 모드 CSS Grid 분기
[5] 상점 연동: seal_sticker 아이템 타입 추가
[6] RP 인장 제거: seal_show_in_rp 기본값 0으로 변경
[7] 모바일 안내: 편집 페이지에 "PC에서 편집" 가드
```
...
| 파일 | 변경 |
|------|------|
| `plugin/morgan/morgan.php` | mg_render_seal() full 모드 CSS Grid 분기 |
| `bbs/seal_edit.php` | GridStack 편집기 UI 추가, 모바일 가드 |
| `bbs/seal_edit_update.php` | seal_layout 저장 처리 |
| `plugin/morgan/install/install.sql` | mg_seal 스키마 업데이트 |
| `db/migrations/` | seal_layout 컬럼 + seal_sticker enum 추가 |
| `bbs/rp_api.php` | 인장 호출 제거 (config 기본값 변경) |

---

*버전: 1.1*
*작성일: 2026-02-10*
*최종 업데이트: 2026-02-20 (그리드 빌더 추가사항)*
*상태: 기본 인장 구현 완료 / 그리드 빌더 미구현*
