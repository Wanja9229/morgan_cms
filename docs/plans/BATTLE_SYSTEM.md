...
3. 기력 서버사이드 재계산 + 차감
4. 데미지/효과 계산
   - **1d6 주사위 판정** → 배율 결정 (dice_lock 등 아이템 효과 먼저 적용)
   - 일반공격/강타: ATK = (str * 2 + 무기ATK) × 주사위배율 - 몬스터DEF
   - 스킬공격: SATK = (dex * 2 + 무기SATK) × 스킬배율 × 주사위배율 - 몬스터DEF
   - 힐링: HEAL = int * 2 * 스킬배율 × 주사위배율
   - 버프/디버프: 주사위 17~20이면 지속 횟수 +1 보너스
   - 도발/수호: Natural 20이면 지속 횟수 +1
   - 크리티컬 판정: rand(0,100) < crit_rate(장비) → 배율 적용 (주사위와 별도)
   - 활성 버프/디버프 보정 적용
5. 효과 적용
...
- 수호 스킬 적용 중이면 반격 데미지 `sk_guard_reduction`% 감소 (기본 30%)
- **HP 스탯 탱커 이점**: HP를 많이 배분하면 DEF = (HP+STR)/4도 올라가므로, 수호 감소율 + 높은 DEF로 반격 피해를 이중 경감
...
모든 전투 행동에 D&D 스타일 **1d20 주사위 판정**을 적용. 기존 ±10% 난수를 주사위 배율로 대체.

**주사위 배율표 (20단계):**

| 주사위 눈 | 배율 | 의미 | 확률 |
|-----------|------|------|------|
| 1 | 0.3x | 대실패 (Critical Fail) | 5% |
| 2 | 0.5x | 실패 | 5% |
| 3 | 0.6x | | 5% |
| 4 | 0.65x | | 5% |
| 5 | 0.7x | 약함 | 5% |
...
| 18 | 1.4x | | 5% |
| 19 | 1.5x | | 5% |
| 20 | 1.8x | 대성공 (Natural 20!) | 5% |

**기대값**: 평균 ≈ **0.96x** — 극단값(1, 20)이 5%로 d6(16.7%)보다 안정적이면서, 20이 터질 때의 폭발력(1.8x!)은 더 극적.

> **D&D 스타일 채택 이유**: 20단계 세밀한 배율로 "조금 잘 침 / 많이 잘 침" 구분 가능.
> 극단값 확률 5%로 밸런스 안정적. 3D 물리 주사위 연출과 궁합이 좋음.

### 14.2 적용 범위

| 행동 | 주사위 적용 | 계산식 |
|------|-----------|--------|
| 일반공격 | ✅ | ATK × 주사위배율 - 몬스터DEF |
| 강타/스킬공격 | ✅ | 파생수치 × 스킬배율 × 주사위배율 - 몬스터DEF |
| 전체공격 | ✅ | 파생수치 × 스킬배율 × 주사위배율 (몬스터별 DEF 감산) |
| 힐링/전체힐 | ✅ | INT파생 × 스킬배율 × 주사위배율 |
| 버프/디버프 | ✅ | 주사위 17~20이면 효과 +1턴 보너스 지속 |
| 도발/수호 | ✅ | 주사위 20이면 +1회 추가 지속 |
| 부활 | ✅ | 부활 HP = INT×3 × 주사위배율 |
| 몬스터 반격 | ❌ | 반격은 고정 계산 (기존 0.8~1.2 랜덤 유지) |

### 14.3 주사위 아이템 (상점 소모품)

| 아이템 | si_type | 효과 | 가격(예시) |
|--------|---------|------|-----------|
| 주사위 고정권 | `battle_consumable` | 다음 1회 주사위를 **20 확정** | 500P |
| 재굴림권 | `battle_consumable` | 행동 후 결과가 불만족이면 **1회 재굴림** | 200P |
| 축복 주사위 | `battle_consumable` | 다음 3회 행동의 주사위 **최솟값 8 보장** (1~7이 안 나옴) | 350P |

**si_effect JSON 예시:**
```json
{"type": "dice_lock", "value": 20, "uses": 1}
{"type": "dice_reroll", "uses": 1}
{"type": "dice_bless", "min_value": 8, "uses": 3}
```

**DB 저장:**
- `mg_battle_slot`에 `dice_effects` (JSON) 컬럼 추가
- 전투 참여 중 아이템 사용 시 해당 슬롯에 효과 기록
- 행동 시 서버에서 효과 소비 + 주사위 결과에 반영
...
```
1. 유저가 행동 버튼 클릭
2. API 전송 → 서버에서 주사위 판정 + 데미지 계산
3. 응답에 dice_result 포함: {dice: 5, multiplier: 1.1, damage: 87, ...}
4. 프론트에서 주사위 굴림 애니메이션 재생 (결과는 서버 값 사용)
5. 결과 표시 (데미지/힐량 + 주사위 눈)
```

**연출 디테일:**
- **@3d-dice/dice-box-threejs** 라이브러리 (출석체크와 동일, 로컬 호스팅)
- Three.js + Ammo.js 물리 시뮬레이션 → 실제 d20 정이십면체가 굴러감
- `MorganDice.roll('1d20@결과값')` → 서버 결정 값으로 강제 착지
- 결과 눈에 따라 색상: 1=빨강, 2~7=주황, 8~13=흰색, 14~17=초록, 18~19=파랑, 20=금색(반짝임)
- **Natural 20** 시 화면 가장자리 금색 플래시 + "NAT 20!" 텍스트
- **Critical Fail (1)** 시 화면 흔들림 효과
- 주사위 고정권 사용 중이면 주사위에 금색 테두리 + "🔒" 아이콘
...
시나리오 A — 보스 막타 노림:
  보스 HP 50 남음, 내 ATK 40
  → 일반공격(주사위 13 이상 필요, 40%확률)
  → 고정권 사용 → 20 확정 → 40×1.8 = 72 → 오버킬 막타!

시나리오 B — 힐러 긴급 회복:
  탱커 HP 위급, 내 INT 15 → 기본 힐 30
  → 주사위 1 나오면 30×0.3 = 9 (치명적!)
  → 축복 주사위로 최소 8 보장 → 30×0.85 = 25 (안전권)

시나리오 C — 스킬 딜러 폭딜:
  SATK 40, 스킬배율 1.5 → 기본 60
  → 고정권(20) + 크리티컬 → 60×1.8×1.5 = 162 → 역대급 폭딜!
```

### 14.6 주사위 관련 DB 변경

```sql
-- mg_battle_slot에 주사위 효과 컬럼 추가
ALTER TABLE mg_battle_slot ADD COLUMN dice_effects TEXT DEFAULT NULL;
-- JSON: [{"type":"dice_lock","value":6,"uses":1}, ...]

-- mg_battle_log에 주사위 결과 컬럼 추가
ALTER TABLE mg_battle_log ADD COLUMN bl_dice INT DEFAULT 0;
-- 0=미적용(반격 등), 1~6=주사위 결과

-- mg_shop_item.si_type에 battle_consumable은 이미 존재
-- 주사위 아이템은 si_effect JSON으로 구분
```
...
| 키 | 기본값 | 설명 |
|----|--------|------|
| `battle_dice_use` | `1` | 주사위 시스템 사용 여부 (0=기존 난수, 1=주사위) |
| `battle_dice_multipliers` | `0.3,0.5,...,1.8` (20개) | 1~20 눈별 배율 (쉼표 구분, 개수=면 수) |
...
| 단계 | 내용 | 의존성 |
|------|------|--------|
| 1 | DB 테이블 7개 생성 + 기존 테이블 수정 + 주사위 컬럼 | — |
| 2 | 스탯/기력 시스템 + 세팅 페이지 | 1 |
| 3 | 몬스터 관리자 CRUD | 1 |
| 4 | 스킬 관리자 CRUD + 기본 스킬 시드 | 1 |
| 5 | 전투 조우 생성 + 탐사 연동 | 1, 3 |
| 6 | 전투 페이지 UI + 참여 시스템 | 2, 5 |
| 7 | 행동 처리 로직 (핵심 전투 엔진 — 도발/주사위 포함) | 2, 4, 6 |
| 8 | 주사위 프론트엔드 (3D CSS 연출, 결과 색상) | 7 |
| 9 | 보상 분배 | 7 |
| 10 | 알림 + 헤더 배너 | 5, 9 |
| 11 | 장비/소모품 상점 연동 + 주사위 아이템 | 2, 7 |
| 12 | 사이드바 메뉴 추가 | 6 |
| 13 | 밸런스 테스트 + 조정 | 전체 |
