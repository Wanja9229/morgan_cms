# Phase 13: 업적 시스템 (Achievement System)
...
| ua_granted_by | varchar(20) NULL | 수동 부여자 (NULL=자동 달성) |
| ua_grant_memo | varchar(255) NULL | 부여 사유 ("대재앙 이벤트 참여" 등) |
...
세계관 이벤트, 특수 공헌 등 자동 판별이 불가능한 업적을 관리자가 직접 부여/회수한다.

**부여 워크플로우:**
1. 업적 관리 페이지에서 type=manual 업적 선택 (또는 자동 업적도 강제 부여 가능)
2. [수동 부여] 버튼 → 모달 오픈
3. 회원 검색 (닉네임/ID, 자동완성) → 단일 또는 복수 선택
4. 부여 사유 입력 (선택, ua_grant_memo에 저장)
5. 일괄 지급 → 보상(포인트/재료/아이템) 동시 처리 + 알림 발송

**회수 워크플로우:**
1. 업적 달성자 목록에서 유저 선택
2. [회수] 버튼 → 확인 모달
3. ua_completed = 0, ua_tier = 0 으로 리셋 (보상은 별도 회수 여부 선택)

**사용 예시:**
- "대재앙을 막은 자" — 특정 이벤트 RP 핵심 역할자에게
- "건국의 아버지" — 세계관 초기 설정 기여 멤버에게
- "1기 마을 주민" — 시즌 1 참여자 전원에게 일괄 부여
- 관리자가 잘못 부여한 업적 회수
