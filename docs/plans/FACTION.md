...
| 테이블 | 용도 |
|--------|------|
| mg_faction_season | 시즌 통합 (익명망 + 점령전 공유. 기간, 상태, 전투 스케줄) |

※ 시즌 = 익명망 코드네임 리셋 + 점령전 기간 + 해킹 진척도 리셋을 모두 포함하는 단일 개념.
...
| mg_card_own | **캐릭터별** 카드 보유 (ch_id 기준) |
| mg_card_deck | 덱 (**ch_id**, deck_type[attack/defense], slot 1~5, card_id) |
| mg_battle_log | 전투 기록 (공격 ch_id, 방어 ch_id, 라운드별 결과, 승패) |
| mg_battle_daily | 일일 전투 횟수 추적 (**mb_id** 기준, 날짜, 사용 횟수) |

> **전투 주체 = 캐릭터**, 카드 보유/덱 = 캐릭터별.
> **일일 전투 제한 = 회원(mb_id) 기준**. 여러 진영에 캐릭터가 있어도 합산 제한.
> 기본 5회, 관리자 설정 가능 (`mg_config('battle_daily_limit', 5)`).
> 중복 카드는 포인트로 환급 (분해).
...
| 테이블 | 용도 |
|--------|------|
| mg_hack_progress | 해킹 진척도 (**진영 단위**: 공격 진영 → 대상 진영, 시즌별 수치) |
| mg_hack_log | 아이템 사용 이력 (개인별 구매 기록, 진영 진척도에 반영) |
| mg_hack_breach | 해킹 성공 기록 (**진영 단위**: 시작 시각, 종료 시각, 접근 허용 진영) |

> 구매 주체 = 개인, 진척도/성공/보상 = 진영 단위.
...
*이 문서는 2차-C 작업 기획입니다. 구현 시 상세 컬럼 정의와 API는 별도 설계.*
