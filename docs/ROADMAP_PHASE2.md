...
| G | 스킨 SDK | 배경/이펙트 파일 기반 커스텀 스킨 | `plans/SKIN_SDK.md` | 중 | 상점 |
| H | 레이아웃 시스템 | 헤더/사이드바/위젯 교체형 멀티 레이아웃 | `plans/LAYOUT_SYSTEM.md` | 중~대 | 없음 (1차 완료 후 착수) |
...
> 배경 레이어 분리 + 파일 기반 커스텀 스킨 시스템.
> 상세: `plans/SKIN_SDK.md` (미작성)

### G.1 스킨 파일 구조
- [ ] `data/skins/{name}/manifest.json + style.css + script.js + preview.png` 규격 정의
- [ ] manifest 스키마 (이름, 버전, 작성자, config_schema)
- [ ] 기존 `bg_effects.php` 하드코딩 이펙트를 스킨 포맷으로 마이그레이션

### G.2 스킨 로더
- [ ] PHP 스킨 로더 (manifest 파싱, CSS/JS 주입)
- [ ] iframe sandbox 격리 (보안)
- [ ] config_schema 기반 파라미터 UI 자동 생성

### G.3 관리자 + 상점 연동
- [ ] 관리자 스킨 관리 (폴더 드롭 → 등록, 미리보기)
- [ ] 상점 아이템으로 연결 등록
- [ ] 유저 적용/해제 (인벤토리 연동)

---

## 2차-H: 레이아웃 시스템 (멀티 레이아웃)

> 현재 Discord 스타일 1종 고정 → 3종 교체형 레이아웃 지원.
> 상세: `plans/LAYOUT_SYSTEM.md` (미작성)

### 현재 구조 (=선행 분리가 필요한 이유)

현재 `head.php`(700줄)에 사이드바 HTML + 2뎁스 패널 JS + 헤더 + 모바일 토글이 일체형으로 들어가 있고,
`app.js`의 `updateSidebar()`가 DOM 셀렉터를 직접 잡고 있으며, 레이아웃 치수(`w-14`, `mr-72`, `h-12`)가
PHP/CSS/JS 전반에 하드코딩되어 있다.

새 레이아웃을 추가하려면 이 결합을 먼저 풀어야 하므로, **Phase 0(선행 분리)**이 필수.

### 목표 레이아웃 3종

| 코드명 | 설명 | 네비게이션 | 위젯 |
|--------|------|-----------|------|
| `discord` | 현재 (디폴트) | 좌측 아이콘바(w-14) + 2뎁스 슬라이딩 패널 | 우측 고정(w-72) |
| `topnav` | 상단 네비형 | 헤더 확장 메가메뉴, 사이드바 없음 | 접이식 또는 하단 |
| `classic` | 전통 사이드바형 | 좌측 텍스트 사이드바(w-56+), 2뎁스 인라인 | 없음 (마이페이지 통합) |

### H.0 Phase 0 — 선행 분리 (8~10h)
- [ ] CSS 변수 추출: `--layout-header-h`, `--layout-nav-w`, `--layout-widget-w` 도입
- [ ] `head.php` 분리: 공통 셸 + 네비 파셜(`_nav_discord.php`, `_nav_topnav.php`, `_nav_classic.php`)
- [ ] `tail.php` 분리: 위젯 파셜(`_widget_sidebar.php`, `_widget_footer.php`, `_widget_none.php`)
- [ ] `app.js` 어댑터 패턴: `updateSidebar()` → 레이아웃별 핸들러 매핑
- [ ] 배경 이펙트 CSS의 하드코딩 offset → CSS 변수 참조로 전환
- [ ] 관리자 설정 `mg_config('layout_type', 'discord')` 추가
- [ ] 검증: discord 레이아웃이 분리 후에도 100% 동일하게 동작하는지 확인

### H.1 Top Nav 레이아웃 (7~8h)
- [ ] `_nav_topnav.php`: 헤더 확장 (메가메뉴 드롭다운, 게시판/세계관 서브메뉴)
- [ ] `_widget_footer.php` 또는 접이식 패널: 유저 정보/인벤토리 배치
- [ ] `app.js` topnav 핸들러: 메가메뉴 활성 상태 관리
- [ ] 모바일 반응형: 햄버거 → 풀스크린 메뉴
- [ ] `#main-content` 마진: `ml-0 mr-0` (풀 폭)

### H.2 Classic 사이드바 레이아웃 (7~8h)
- [ ] `_nav_classic.php`: 좌측 텍스트 사이드바(w-56), 게시판/세계관 인라인 펼침
- [ ] `_widget_none.php`: 위젯 없음 (마이페이지에 통합)
- [ ] `app.js` classic 핸들러: 아코디언 펼침/접기 상태 관리
- [ ] 모바일 반응형: 오버레이 사이드바 (기존과 유사)
- [ ] `#main-content` 마진: `ml-56 mr-0`

### H.3 레이아웃 전환 UI
- [ ] 관리자 설정 페이지에 레이아웃 선택 UI (미리보기 카드)
- [ ] 레이아웃 변경 시 세션/캐시 클리어 안내
- [ ] (선택) 유저별 레이아웃 선택 허용 — `mg_seal` 또는 별도 테이블

### 작업량 추정

| 단계 | 예상 시간 | 비고 |
|------|-----------|------|
| H.0 선행 분리 | 8~10h | **가장 중요, 리그레션 주의** |
| H.1 Top Nav | 7~8h | 수요가 가장 높을 레이아웃 |
| H.2 Classic | 7~8h | H.1 완료 후 패턴 재사용 |
| H.3 전환 UI | 2~3h | H.1 이후 가능 |
| QA (3종 × 3뷰포트) | 3~4h | 단계별 증분 테스트 |
| **합계** | **27~33h** | |

### 권장 진행 순서

1. **1차 배포 전**: CSS 변수 추출만 선행 (3h) — 하드코딩 제거, 리스크 최소
2. **1차 안정화 후**: H.0 완료 → H.1(Top Nav) 추가 — 실질적 2종 레이아웃
3. **2차 중반**: H.2(Classic) + H.3(전환 UI) — 3종 완성
...
## 추후 검토: 히든 이벤트 고도화 (Phase 3)

> 히든 이벤트 시스템 Phase 1 구현 완료 (2026-03-04).
> Phase 2(모니터링)는 Phase 1에서 선반영 완료.
> 아래 항목은 운영 데이터 축적 후 필요에 따라 검토.
> 기획서: `docs/add_plans/PLAN_EVENT_ANTIABUSE.md`

| 항목 | 설명 | 비고 |
|------|------|------|
| 시간대별 활성 | `time_start`/`time_end` 컬럼 + 관리자 UI + API 조건 | 예: 20시~23시만 출현 |
| 유저별 이벤트 차단 | 특정 유저를 히든 이벤트에서 일시 제외 | 차단 테이블 or 유저 플래그 |
| 보상 회수 | 관리자가 수령 로그에서 선택 → 포인트/재료 차감 | 부정 수령 대응 (수동) |
...
| 2026-02-27 | 2차-G 스킨 SDK, 2차-H 레이아웃 시스템 추가 |
| 2026-03-04 | 히든 이벤트 고도화(Phase 3) 추후 검토 항목 추가 |
