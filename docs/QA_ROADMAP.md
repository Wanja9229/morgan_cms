...
| 단계 | 주체 | 내용 |
|------|------|------|
| **1. 코드 검수** | Claude | 코드 리딩, 버그 수정, 반응형 CSS 보완, SQL/보안 점검 |
| **2. 사용자 확인** | 사용자 | 브라우저에서 직접 조작, 의도와 다른 구현 체크, UX 개선점 피드백 |

**[x] 완료 = 두 단계 모두 통과.** 코드만 본 상태에서는 [x] 처리하지 않음.
사용자 확인 시 개선 요청이 나오면 반영 후 [x] 처리.
...
|------|------|------|
| 캐릭터 목록/검색 | [ ] | |
| 승인/반려 워크플로우 | [ ] | ⚠️ 반려(rejected) 상태 미구현 |
| 반려 사유 입력 + 알림 | [ ] | rejected 상태 + 반려 사유 필드 필요 |
| 프론트 캐릭터 등록/수정 | [ ] | |
| 프론트 캐릭터 목록/상세 | [ ] | |
...
| 필드 정렬 순서 | [ ] | |
| 프론트 캐릭터 등록폼 연동 | [ ] | |
| 반응형 | [x] | 그리드 min-width + overflow-x:auto 추가 |
...
| 프론트 인장 편집/미리보기 | [ ] | |
| 게시글/캐릭터 프로필 인장 표시 | [ ] | |
| 반응형 | [x] | overflow-x:auto + min-width, 모달 max-height 추가 |
