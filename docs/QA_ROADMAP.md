...
| 관계 신청/승인/거절 | [x] | 전체 플로우 + 알림 + 업적 트리거, 관리자 자기 캐릭터 간 신청 허용 |
| 관계 로그 필수 설정 | [x] | relation_require_log ON/OFF, 관계 관리 페이지에서 설정 |
| accepted 일괄 active 전환 | [x] | 관리자 관계 관리에서 일괄 전환 버튼 |
...
| 항목 | 상태 | 비고 |
|------|------|------|
| SPA 라우터 (app.js) | [x] | 사이드바 활성 상태 동기화 완료, 상대 URL resolve 버그 수정 |
| 사이드바 네비게이션 | [x] | 검수에서 전체 정리됨 |
| 헤더 (접속자 수, 알림 벨) | [-] | 접속자 드롭다운 + 뱃지 추가됨 |
| 반응형 레이아웃 (모바일/태블릿) | [ ] | 전체 페이지 대상 확인 필요 |
| 페이지 타이틀 형식 통일 | [x] | "사이트명 \| 페이지명" |
| 로그인/회원가입 스킨 | [ ] | |
| 마이 페이지 허브 (mypage.php) | [-] | 캐릭터 관리 링크 character→character.php 변경, 관리자 버튼→dashboard |
...
| 03-18 | 기능+QA | **멀티테넌트 상점 시드 동기화**: install.sql 누락 아이템 72개 추가(프로필 스킨/이펙트/배경, 인장, 칭호뽑기, 캐릭터슬롯, 이모티콘등록권), seed.sql 잘못된 카테고리 제거, 기존 테넌트용 마이그레이션 추가. **세계관 위키**: 목록 조회수 제거→한줄요약(la_summary) 표시. **캐릭터 신청**: 기간 미설정 시 승인 캐릭터 수정 항상 허용. **관계 시스템**: 관리자 자기 캐릭터 관계 신청 허용($is_admin 예외), 캐릭터 상세→목록 사이드바 포커싱 통일. 캐릭터 프로필 상단 전신 이미지(ch_image) 표시. rellog 게시판 글쓰기 시 대상 관계 select 추가(mg_cr_id)+캐릭터 자동 선택. 관계 로그 필수 여부 관리자 설정(relation_require_log, OFF→수락 즉시 active). **내 캐릭터 페이지 관계 탭**: character.php에 캐릭터/관계 2탭 추가(받은 신청+로그 대기+보낸 신청+활성 관계). 마이페이지 캐릭터 관리 링크 character_list→character 변경. **사이드바 포커싱 버그**: app.js updateSidebar URL resolve 기준 origin→href 변경(상대 URL 정상 처리). 관리자 버튼 링크 config→dashboard 변경. **관계 관리 페이지 개편**: relation_require_log 설정을 config→relation.php로 이동, accepted 일괄 active 전환 버튼+통계에 로그 대기 건수 추가 |
