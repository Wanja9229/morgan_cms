...
### 서버 스키마 불일치 수정
- [x] mg_character: 프로필 스킨 5컬럼 누락으로 UPDATE 전체 무음 실패 — ALTER TABLE로 해결
- [x] 원인: 마이그레이션 SQL 서버 미적용 (초기 테넌트 세팅 시점과 스키마 변경 시점 불일치)
...
### 룰렛 시스템 → `plans/ROULETTE.md`
- [ ] DB: mg_roulette_prize, mg_roulette_log (2테이블)
- [ ] 관리자: 보상/벌칙 항목 CRUD + 설정 (비용, 제한, 잭팟 등)
- [ ] 스핀 로직 (가중치 확률, 포인트 차감, 보상 즉시 지급)
- [ ] 룰렛 휠 UI (CSS/JS 애니메이션, 탭 전환: 휠/결과 피드)
- [ ] 잭팟 풀 (누적 표시, 0.001% 확률 전액 지급)
- [ ] 벌칙: 닉네임/프사 시스템 강제 (페이지 로드 시 만료 체크, 크론 불필요)
- [ ] 벌칙 로그 게시판 (전용 게시판, 미제출 시 다음 스핀 차단)
- [ ] pending→active 전환 (알림 읽음 즉시 OR 24시간 경과 시 자동)
- [ ] 상점 아이템 3종 (무효화, 랜덤 떠넘기기, 지목 떠넘기기)
- [ ] 알림 연동 (벌칙/떠넘기기/잭팟)
...
### 마이그레이션 MySQL 호환성 수정
- [x] `ADD COLUMN IF NOT EXISTS` → `information_schema + PREPARE/EXECUTE` 패턴으로 11개 마이그레이션 전환
- [x] 마이그레이션 작성 가이드 문서 (`docs/MIGRATION_GUIDE.md`)
- [x] CLAUDE.md에 금지 구문 명시

### 신규 테넌트 프로비저닝 수정
- [x] `install.sql`에 전투 시스템 7테이블 + 훈련 시스템 3테이블 추가
- [x] 기존 테넌트 누락 컬럼 15건 수동 적용 (8테이블)
- [x] 기본 설정 fallback 수정: `battle_use`, `battle_training_use`, `pioneer_use` 기본값 '0' → '1'
- [x] `mg_pioneer_enabled()` 키 불일치 수정 (`pioneer_enabled` → `pioneer_use`)
- [x] 멀티테넌트 `tenants` 테이블 누락 컬럼 3건 확인 (`r2_enabled`, `custom_domain`, `storage_used_mb`)

### 관리자 폼 CSS 표준화
- [x] 전투/스킬/몬스터 관리자 폼 → `mg-form-input`/`mg-form-select`/`mg-form-textarea` 통일
- [x] 반응형 그리드, 섹션 구분, 버튼 레이아웃 표준화

### 캐릭터 시스템 개선
- [x] CON/LUK 스탯 프론트 숨김 + API 조작 차단 (DB 컬럼 유지, 향후 전투 시스템용 예약)
- [x] 캐릭터 뷰페이지 스탯 항상 표시 (데이터 없으면 기본값 사용)
- [x] 캐릭터 목록에 "캐릭터 신청" 버튼 추가
- [x] 캐릭터 신청 기간 설정 (관리자 캐릭터 관리 페이지 → AJAX 저장)
- [x] "모집 중단" 체크박스 (기간 무관 즉시 차단)
- [x] 전투 스탯을 12개 프로필 스킨 내부에 통합 (스킨별 디자인 톤 반영)
- [x] 전투 스탯 UI를 캐릭터 수정 폼에서 기본정보 아래로 이동
- [x] 승인된 캐릭터 수정 제한 (기본정보/이미지/프로필필드 읽기전용, 칭호/스킨/배경만 수정 가능)
- [x] 신청 기간 중 승인 캐릭터 프로필 수정 허용 (`$_is_edit_period` 기반, 기간 외 잠금 유지)
- [x] 캐릭터 수정 이력 추적 (`mg_character_edit_log` 테이블, 대시보드+캐릭터 상세 조회)
- [x] 프로필 스킨 "기본 스킨" 저장 버그 수정 (default 값 검증 통과)
- [x] 배경 이펙트 "없음" 옵션 추가 + si_type 검증 버그 수정 (profile_bg → profile_effect)
- [x] 관계도 배치 저장 버튼을 관계도 내부 우상단 오버레이로 이동

### 전투 시스템 — 글로벌 HP
- [x] HP를 per-slot → per-character (`mg_battle_energy.current_hp/max_hp`)로 전환
- [x] 글로벌 HP 함수: `get_global_hp`, `damage_global_hp`, `heal_global_hp`, `revive_global_hp`, `sync_max_hp`
- [x] Lazy HP regen (접근 시 시간 기반 자동 회복)
- [x] 전투 API 전체 글로벌 HP 전환 (참가/공격/힐/부활/아이템/폴링)
- [x] 캐릭터 프로필 뷰 + 전체 20개 스킨에 HP바 추가 (전투 시스템 토글 연동)

### 프로필 스킨 확장
- [x] 신규 프로필 스킨 8종 (Windows 98, DOS 터미널, macOS 모던, ECHO-4, VS Code, JRPG, 매거진, 전설 카드)
- [x] 상점 아이템 등록 + 마이그레이션

### 상점 아이템 — 스탯 초기화권
- [x] `stat_reset` 아이템 타입 등록 + 시드 상품 (500P, 소모품)
- [x] `mg_stat_reset()` — 수업 보너스 유지, 유저 배분 포인트만 회수 → `stat_locked=0`
- [x] 인벤토리 캐릭터 선택 모달 → 초기화 실행 → 재배분 가능

### 관리자 메뉴 개편
- [x] 파견 시스템을 개척에서 분리하여 독립 그룹화 (개척/파견/전투 3그룹)
- [x] 전투 설정 탐사 확률 UI 숨김 (dead config, 실제는 파견 이벤트 시스템 사용)
- [x] 파견지↔파견 이벤트↔전투 설정 간 상호 링크 가이드 텍스트 추가
- [x] 모바일 우측 사이드바(widget-sidebar) z-index 수정 (백드롭 아래 렌더링 버그)

### 프로필 필드 관리 개선
- [x] 필드/섹션 추가 시 동일 이름 중복 체크 (서버+클라이언트)
- [x] 각 필드 행에 개별 삭제 버튼 (휴지통 아이콘)
- [x] 섹션 삭제 버튼 추가 (포함 필드+값 전체 삭제)
- [x] 삭제 전 confirm 경고 메시지 통일
- [x] 섹션 헤더 버튼 스타일 개선 (필드 추가=primary, 이름 변경=secondary, 섹션 삭제=danger)

### 기타
- [x] 메인 기본 화면 정리 (그누보드 환영 멘트 + 플레이스홀더 카드 제거)
- [x] JS/CSS 캐시 버스팅 (`G5_JS_VER` / `G5_CSS_VER` 갱신)

### 개척 해금 타입 확장 (Phase 10 확장)
> 1차 완료 후 늘어난 시스템들을 개척 시설 해금에 추가 연동.
> 기존 패턴(`mg_is_unlocked()` + 진입점 체크)과 동일한 방식.

- [ ] `expedition` — 탐험가 길드/파견 사무소 → 파견 시스템 해금
- [ ] `concierge` — 중개소 → 의뢰 시스템 해금
- [ ] `seal` — 인장 공방/각인소 → 인장 편집 해금
- [ ] `radio` — 라디오 타워/방송국 → 라디오 위젯 해금
- [ ] `emoticon_create` — 화방/공예소 → 이모티콘 자체 제작 해금 (선택)
...
| 2026-03-13 | 룰렛 시스템 기획 확정 (보상/벌칙/잭팟/떠넘기기, plans/ROULETTE.md) |
