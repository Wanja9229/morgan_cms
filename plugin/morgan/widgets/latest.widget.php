<?php
/**
 * Morgan Edition - Latest Posts Widget
 *
 * 게시판의 최신 글 목록 표시
 */
...
class MG_Latest_Widget extends MG_Widget_Base {
    protected $type = 'latest';
    protected $name = '최신글';
    ...
    public function render($config) {
        ...
        $posts = array();
        if ($config['bo_table']) {
            $posts = $this->getLatestPosts($config['bo_table'], (int)$config['rows']);
        }
        ...
        return $this->renderSkin(array(
            ...
            'show_writer' => $config['show_writer']
        ));
    }

    private function getLatestPosts($bo_table, $limit = 5) {
        global $g5;
        ...
        $sql = "SELECT wr_id, wr_subject, wr_name, wr_datetime
                FROM {$write_table}
    ...
    public function renderConfigForm($config) {
        ...
        <div class="mg-form-group">
            <label class="mg-form-label">작성자 표시</label>
            <select name="widget_config[show_writer]" class="mg-form-select">
                <option value="1" <?php echo $config['show_writer'] ? 'selected' : ''; ?>>표시</option>
                <option value="0" <?php echo !$config['show_writer'] ? 'selected' : ''; ?>>숨김</option>
            </select>
        </div>
        <?php
        return ob_get_clean();
...
// 팩토리에 등록
MG_Widget_Factory::register('latest', 'MG_Latest_Widget');
