...
-- 4.7 프로필 이펙트 (Vanta.js, sc_id = 꾸미기)
INSERT INTO `mg_shop_item` (`sc_id`, `si_name`, `si_desc`, `si_price`, `si_type`, `si_effect`, `si_stock`, `si_consumable`, `si_display`, `si_use`, `si_order`)
...
-- 4.15 이모티콘 등록권 + 추가 의뢰권 (sc_id = 이용권)
INSERT INTO `mg_shop_item` (`sc_id`, `si_name`, `si_desc`, `si_price`, `si_type`, `si_effect`, `si_stock`, `si_consumable`, `si_display`, `si_use`)
...
CREATE TABLE IF NOT EXISTS `mg_relation` (
    ...
    `cr_status` enum('pending','accepted','active','rejected') NOT NULL DEFAULT 'pending' COMMENT '상태',
    `cr_datetime` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '신청일',
    `cr_accept_datetime` datetime DEFAULT NULL COMMENT '승인일',
    `cr_wr_id_a` int DEFAULT NULL COMMENT 'A쪽 관계 로그 글 ID',
    `cr_wr_id_b` int DEFAULT NULL COMMENT 'B쪽 관계 로그 글 ID',
    `cr_bo_table` varchar(20) DEFAULT NULL COMMENT '관계 로그 게시판',
    PRIMARY KEY (`cr_id`),
...
CREATE TABLE IF NOT EXISTS `mg_battle_slot` (
    ...
    `bs_action_count` INT DEFAULT 0,
    `buffs_active`    TEXT,
    `dice_effects`    TEXT DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `mg_battle_log` (
    ...
    `bl_dice`         INT DEFAULT 0,
    `bl_value`        INT DEFAULT 0,
    `bl_detail`       TEXT,
...
INSERT IGNORE INTO mg_shop_item (si_type, si_name, si_desc, si_price, si_image, si_effect, si_use, sc_id, si_consumable, si_display)
SELECT 'roulette_transfer_random', '랜덤 패스권', '룰렛 벌칙을 랜덤 회원에게 전달합니다.', 200, '', '{}', 1,
 COALESCE((SELECT sc_id FROM mg_shop_category WHERE sc_name = '이용권' LIMIT 1), 0), 1, 1 FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM mg_shop_item WHERE si_type = 'roulette_transfer_random');

INSERT IGNORE INTO mg_shop_item (si_type, si_name, si_desc, si_price, si_image, si_effect, si_use, sc_id, si_consumable, si_display)
SELECT 'roulette_transfer_target', '지목 패스권', '룰렛 벌칙을 특정 회원에게 전달합니다.', 400, '', '{}', 1,
 COALESCE((SELECT sc_id FROM mg_shop_category WHERE sc_name = '이용권' LIMIT 1), 0), 1, 1 FROM DUAL
...
('100 포인트', 'reward', 'point', '100', 30, '#4ade80', 'coins', '100 포인트를 획득합니다', 0, 0, 1, 1),
('300 포인트', 'reward', 'point', '300', 15, '#22d3ee', 'gem', '300 포인트를 획득합니다', 0, 0, 1, 2),
('500 포인트', 'reward', 'point', '500', 5, '#a78bfa', 'star', '500 포인트를 획득합니다', 0, 0, 1, 3),
('꽝', 'blank', 'none', '', 25, '#6b7280', 'wind', '아쉽지만 꽝입니다', 0, 0, 1, 4),
('닉네임 변경', 'penalty', 'nickname', '{"nickname":"🐔치킨러버"}', 10, '#f87171', 'flame', '닉네임이 강제 변경됩니다', 24, 1, 1, 5),
('프사 변경', 'penalty', 'profile_image', '', 8, '#fb923c', 'image', '프로필 이미지가 강제 변경됩니다', 24, 1, 1, 6),
('잭팟!', 'jackpot', 'none', '', 2, '#fbbf24', 'crown', '축적된 잭팟 포인트를 모두 획득합니다!', 0, 0, 1, 7),
('50 포인트', 'reward', 'point', '50', 5, '#86efac', 'circle-dollar-sign', '50 포인트를 획득합니다', 0, 0, 1, 8);
